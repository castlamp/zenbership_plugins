...
class mailReader {
    ...
    public function __construct(){
        ...
        $this->allowed_senders = '';
        $this->pdo = '';;
        */
    }

    public function setSaveDir($save_directory)
    {
        if(!preg_match('|/$|',$save_directory)){ $save_directory .= '/'; } // add trailing slash if needed
    ...
    {
        return $this->decoded->headers;
    }

    public function getTo()
    {
        if (! empty($this->decoded->headers['to'])) {
            return $this->decoded->headers['to'];
        }
    ...
    {
        if (! empty($this->decoded->headers['cc'])) {
            return $this->decoded->headers['cc'];
        }
    ...
    {
        if (! empty($this->decoded->headers['X-Spam-Status'])) {
            return $this->decoded->headers['X-Spam-Status'];
        }
    ...
    private function saveFile($filename,$contents,$mimeType = 'unknown')
    {
        ...
        while(!$unlocked_and_unique){
            // Find unique
            $id = md5(time() . "_" . $filename);
            $name = $id . '.' . $extUse;
            while(file_exists($this->save_directory . $name)) {
                $name = md5(time() . "_" . $filename) . '.' . $extUse;
            }

            // Attempt to lock
            $outfile = fopen($this->save_directory.$name.'.'.$extUse,'w');
